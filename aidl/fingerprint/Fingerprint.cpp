...
SensorLocation Fingerprint::getSensorLocation() {
    ...
    if (dim.size() != 3 and dim.size() != 4) {
        if (!loc.empty()) ALOGE("Invalid sensor location input (x|y|radius) or (x|y|radius|display): %s", loc.c_str());
    } else {
        int32_t x, y, r;
        std::string d = "";
...
ndk::ScopedAStatus Fingerprint::createSession(int32_t /*sensorId*/, int32_t userId,
    ...
    mSession = SharedRefBase::make<Session>(getSensorHal(), mUdfpsHandler, userId, cb, mLockoutTracker);
    *out = mSession;
